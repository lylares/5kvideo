...
$result = mysqli_query($conn,"select * from wudu_tvlive where tl_id = $pid");
mysqli_num_rows($result);
while($row = mysqli_fetch_array($result)){
	$play = $row['tl_url'];
	$playname = $row['tl_name'];
	$playimg = $row['tl_img'];
}
	...
	<div class="wdu-part-case">
		<div class="wdu-play-info wdu-part-rows wdu-back-whits wdu-marg-top">
			<div class="wdu-play-player wdu-part-rows wdu-back-black wdu-part-16by9 wdu-part-19by9">
				...
				<iframe id="wdu-play-iframe" src="https://vod.parse.5kvideo.cc/?url=<?php echo $play;?>" class="wdu-play-iframe wdu-part-full" frameborder="0" scrolling="no" allowfullscreen="true" data-name="<?php echo $playname;?>" >
				</iframe>
...
<script type='text/javascript' src="<?php echo $wudu_domain;?>theme/wudu3.0/asset/js/store.js"></script>
<script>
init()
function init() {
	if (!store.enabled) {
...
var playId = "<?php echo str_replace('play=','',$_SERVER['QUERY_STRING']);?>",playCover = "<?php echo $d_picture;?>",playTitle = "<?php echo $d_name;?>",playDefaultUrl = $("#play_default0").attr("data-href");
var playDefaultepd = $("#play_default0").attr("data-num"),vodSource = $("#defaultEspPanel").data('name'),esptotalnum ='<?php echo $esptotlaNum;?>',spid="<?php echo $d_id;?>";
var videoInfo = {
	types: "splay",
    id: playId,
	name: playTitle,
...
function refreshVideo() {
    ...
    $("#wuduPlayer").attr("src", _wdu_playinfo.videoApi + videoInfo.url);
