					...
					<div class="wdu-tabs-boxs">
						<div class="wdu-tabs-head">
							<span class="wdu-tabs-top wdu-tabs-title wdu-font-xvi wdu-part-rows wdu-hidden wdu-show">
							    <span class="wdu-col-xs4 wdu-part-eone"><?php if($type=='tv'){echo'选集';}else{echo'播放源';}?></span>
							</span>
						</div>
						<div class="wdu-tabs-foot">
							...
							<?php 
                              if(isset($_SESSION['uid'])){
											...
											$DBdata=$db->getALL("select * from wudu_vod where pid ='$pid'");
	                                        foreach ($DBdata as $key =>$row){
                                                $scontent=explode("\r\n",$row['scontent']);
												
	                                            $esptotlaNum = count($scontent);
                                                for($i=0;$i<$esptotlaNum;$i++)
                                                {	
                                                    $scontent[$i]=explode('$',$scontent[$i]);
...
//
                                               if($scontent[$i][1]!=""){
											    ...
											    <li class="wdu-padding wdu-col-xs3 wdu-col-md6 wdu-col-lg4">
												    <a data-num="'.($i+1).'" 
                                                    class="ea-site wdu-btns-info wdu-rims-info wdu-part-eone wdu-tabs-btn"
                                                    data-href="'.htmlentities(trim($scontent[$i][1])).'">'.$dname.'</a>
												</li>
												';
                                                }
                                                }
    ...
    if (store.enabled) {
        ...
        for(var i=0; i<histemp.length; i++) {
            if(histemp[i].id == window.globalConfig.playInfo.id) {
                ...
                if(window.globalConfig.playInfo.types =="tv"){
					
					layer.msg("您上次观看到第 " + histemp[i].episode + " 集");
					
				}else if(window.globalConfig.playInfo.types =="mv"){
	...
	$(".ea-site").on("click",function() { 
        ...
        refreshVideo();
		 if(window.globalConfig.playInfo.types =="tv"){
					...
					layer.msg("正在播放" + window.globalConfig.playInfo.vodSource + " 源");
...
$(function() {
 ...
 $.ajax({
			type: "POST",
			url: window.globalConfig.api.getTvlist,
...
$play = trim($_GET['play']);
  ...
  $playurl=$wudu_domain.'splay/'.$pid.'.html';
if(isset($play)&&$play)
{
